...
// The idea is to use a regular expression to match the first number in the string.
// If the first number is not found, return 0.
// If the first number is found, check if it is in the range [(-2) ** 31, 2 ** 31 - 1].
// If it is not in the range, return the closest number in the range.
// If it is in the range, return the number.
